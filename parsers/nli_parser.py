"""Adapted from https://github.com/osainz59/Ask2Transformers/blob/d7b73fe475a0bbf449d63b7b14442c302d28da30/
a2t/legacy/relation_classification/mnli.py"""
...
from data_loader.general_domain.to_nli import *
...
class _NLIRelationClassifier(Classifier):
    ...
    def _initialize(self, pretrained_model):
        ...
        if self.ent_pos is None:
            try:
                self.ent_pos = self.config.label2id.get("2")
            except ValueError:
    ...
    def __call__(
        ...
        for i, feature in tqdm(enumerate(features), total=len(features)):
            sentences = [
                f"{feature.premise} {self.tokenizer.sep_token} {label_template.format(child=feature.subj_child, parent=feature.obj_parent)}."
                for label_template in self.labels
            ]
            # sentences:
            # ['<ROOT> October 29 , 2012 . </s> October 29 , 2012 and <ROOT> are not related.',
            #  '<ROOT> October 29 , 2012 . </s> October 29 , 2012 happened before <ROOT>.',
            #  '<ROOT> October 29 , 2012 . </s> October 29 , 2012 happened after <ROOT>.',
            #  '<ROOT> October 29 , 2012 . </s> October 29 , 2012 happened at around the same time as <ROOT>.',
            #  '<ROOT> October 29 , 2012 . </s> October 29 , 2012 happened <ROOT>.',
            #  '<ROOT> October 29 , 2012 . </s> October 29 , 2012 depended on <ROOT>.']
    ...
    def _apply_valid_conditions(self, probs, features: List[NLIInputFeature]):
...
class NLIRelationClassifierWithMappingHead(_NLIRelationClassifier):
    ...
    def __call__(self, features: List[NLIInputFeature], batch_size=1, multiclass=True):
    ...
    def decode_tdg(self, features: List[NLIInputFeature], batch_size=1, multiclass=True):
        outputs = super().__call__(features, batch_size, multiclass)
        if self.valid_conditions:
            outputs = self._apply_valid_conditions(outputs, features)
