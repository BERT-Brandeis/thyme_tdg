...
EVENT_TIMEX_BIO2id = {'b_e': 0, 'i_e': 1, 'b_t': 2, 'i_t': 3, 'o': 46}
id2EVENT_TIMEX_BIO = {v: k for k, v in EVENT_TIMEX_BIO2id.items()}
...
class Node:
    def __init__(self, snt_index_in_doc=-1, start_word_index_in_snt=-1, end_word_index_in_snt=-1, node_index_in_doc=-1,
                 ...
                 thyme_id=-1, note_idx=None, end_snt_index_in_doc=None, node_id=-1):
        ...
        if self.start_word_index_in_doc < 0:
...
def get_no_ref_event_node():
    return Node(-5, -5, -5, -5, -5, -5, NO_REF_EVENT_word, NO_REF_EVENT_label, -5, -5)


def get_padding_node():
    return Node(-2, -2, -2, -2, -2, -2, PAD_word, PAD_label, -2, -2)


def get_dct_node():
    return Node(-7, -7, -7, -7, -7, -7, DCT_word, DCT_label, -7, -7)
